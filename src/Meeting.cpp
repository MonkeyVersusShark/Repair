...
*/
std::string Meeting::getSponsor(void) const { return this->m_sponsor; }
...
*/
std::string Meeting::getTitle(void) const { return this->m_title; }

/**
* @brief  set the title of a meeting
* @param  the new title of a meeting
*/
void Meeting::setTitle(const std::string t_title) { this->m_title = t_title; }
...
bool Meeting::isParticipator(const std::string t_username) const {
  for (auto index : m_participators) {
    if (index == t_username)
      return true;
  }
  return false;
