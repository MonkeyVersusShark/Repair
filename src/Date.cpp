...
bool Date::isValid(const Date t_date) {
  ...
  if (t_date.m_month <= 0 || t_date.m_month > 12)
    return false;
  if (t_date.m_day <= 0 || t_date.m_day > 31)
    return false;
  if ((t_date.m_month % 2 == 1 && t_date.m_month < 8) ||
      (t_date.m_month % 2 == 0 && t_date.m_month >= 8)) {
    if (t_date.m_day > 31)
      return false;
    ...
    if (t_date.m_day > 30)
      return false;
  } else if ((t_date.m_year % 4 == 0 && t_date.m_year % 100 != 0) ||
             t_date.m_year % 400 == 0) {
    if (t_date.m_day > 29)
      return false;
  } else {
...
Date Date::stringToDate(const std::string t_dateString) {
  ...
  if (!isdigit(t_dateString[0]) || !isdigit(t_dateString[1]) ||
      !isdigit(t_dateString[2]) || !isdigit(t_dateString[3]) ||
      t_dateString[4] != '-' || !isdigit(t_dateString[5]) ||
      !isdigit(t_dateString[6]) || t_dateString[7] != '-' ||
      !isdigit(t_dateString[8]) || !isdigit(t_dateString[9]) ||
      t_dateString[10] != '/' || !isdigit(t_dateString[11]) ||
      !isdigit(t_dateString[12]) || t_dateString[13] != ':' ||
