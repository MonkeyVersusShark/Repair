...
bool AgendaService::createMeeting(const std::string userName,
  ...
  for (auto iterator = participator.begin(); iterator != participator.end();
       iterator++) {
  ...
  if (date1 >= date2)
    return false;
  if (!Date::isValid(date1) || !Date::isValid(date2))
    return false;
  auto filter_1 = [&](const User &user) -> bool {
    return user.getName() == userName;
  };
...
AgendaService::meetingQuery(const std::string userName,
  ...
  auto filter = [&](const Meeting &meeting) -> bool {
    if (meeting.getSponsor() == userName || meeting.isParticipator(userName)) {
      return (!(meeting.getStartDate() > endDate) &&
              !(meeting.getEndDate() < startDate));
    }
    return false;
  };
